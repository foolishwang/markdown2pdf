import { u as usePathname, R as React, g as getLayoutSegmentContext, r as reactExports, a as getDefaultExportFromCjs } from "../index.js";
...
function Header({ theme, onToggleTheme, locale, t }) {
  const handleLanguageChange = (e) => {
    const newLocale = e.target.value;
    window.location.href = `/${newLocale}`;
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "header", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "header-left", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "logo", title: "MK2PDF — Markdown to PDF Editor", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "logo-icon", children: "📝" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "logo-text", children: "MK2PDF" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "logo-badge", children: t.logoBadge })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "header-right", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "language-switcher", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          value: locale,
          ...
          children: locales.map((l) => /* @__PURE__ */ jsxRuntimeExports.jsxs("option", { value: l, children: [
            localeFlags[l],
            " ",
            localeNames[l]
          ] }, l))
...
async function generatePdf(sourceElement, options) {
  const html2pdf = (await import("./html2pdf-B17q2Pmy.js").then((n) => n.h)).default;
...
async function exportDocument(sourceElement, markdown2, settings) {
  ...
  switch (settings.format) {
    ...
    case "doc":
      ...
      break;
    case "rtf":
      blob = new Blob([markdownToRtf(markdown2)], { type: "application/rtf;charset=utf-8" });
      break;
    case "txt":
      blob = new Blob([markdownToPlainText(markdown2)], { type: "text/plain;charset=utf-8" });
      break;
    case "md":
      blob = new Blob([markdown2], { type: "text/markdown;charset=utf-8" });
      break;
    case "json":
      blob = new Blob([JSON.stringify(markdownToJson(markdown2), null, 2)], {
        type: "application/json;charset=utf-8"
      });
      break;
    case "csv":
      blob = new Blob([markdownTablesToCsv(markdown2)], { type: "text/csv;charset=utf-8" });
      break;
    case "xml":
      blob = new Blob([markdownToXml(markdown2)], { type: "application/xml;charset=utf-8" });
      break;
    default:
...
async function convertFileToMarkdown(file) {
  const extension2 = getExtension(file.name);
  const sourceFormatLabel = SOURCE_LABELS[extension2] ?? "Text";
  const raw = await file.text();
  if (["md", "markdown", "mdown", "mkd", "mdx", "txt", "text"].includes(extension2)) {
  ...
  if (extension2 === "csv" || extension2 === "tsv") {
    return {
      markdown: delimitedTextToMarkdownTable(raw, extension2 === "tsv" ? "	" : ","),
      sourceFormatLabel
    };
  }
...
function ensureExtension(filename, extension2) {
  return filename.toLowerCase().endsWith(`.${extension2}`) ? filename : `${filename}.${extension2}`;
}
...
function markdownToPlainText(markdown2) {
  return markdown2.replace(/```[\s\S]*?```/g, (block) => block.replace(/```[a-zA-Z0-9_-]*\n?|\n?```/g, "")).replace(/`([^`]+)`/g, "$1").replace(/!\[([^\]]*)\]\([^)]+\)/g, "$1").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").replace(/^>\s?/gm, "").replace(/^#{1,6}\s+/gm, "").replace(/^[-*+]\s+/gm, "• ").replace(/^\d+\.\s+/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/~~([^~]+)~~/g, "$1").replace(/\n{3,}/g, "\n\n").trim();
}
function markdownToJson(markdown2) {
  const headings = Array.from(markdown2.matchAll(/^(#{1,6})\s+(.+)$/gm)).map((match) => ({
    level: match[1].length,
    text: match[2].trim(),
    slug: slugify(match[2])
  }));
  const links = Array.from(markdown2.matchAll(/\[([^\]]+)\]\(([^)]+)\)/g)).map((match) => ({
    text: match[1],
    href: match[2]
  }));
  const lines = markdown2.split("\n");
  const words = markdown2.trim().split(/\s+/).filter(Boolean).length;
  return {
    type: "markdown-document",
    exportedAt: (/* @__PURE__ */ new Date()).toISOString(),
    stats: {
      characters: markdown2.length,
      words,
      lines: lines.length
    },
    headings,
    links,
    content: markdown2,
    plainText: markdownToPlainText(markdown2)
  };
}
function markdownTablesToCsv(markdown2) {
  const tables = extractMarkdownTables(markdown2);
  if (tables.length === 0) {
    return "No markdown tables found";
  }
  return tables.map(
    (table2) => table2.map((row2) => row2.map((cell2) => escapeCsv(cell2)).join(",")).join("\n")
  ).join("\n\n");
}
function markdownToXml(markdown2) {
  const headings = Array.from(markdown2.matchAll(/^(#{1,6})\s+(.+)$/gm)).map((match) => ({
    level: String(match[1].length),
    text: match[2].trim()
  }));
  const paragraphs = markdownToPlainText(markdown2).split(/\n{2,}/).map((item) => item.trim()).filter(Boolean);
  return `<?xml version="1.0" encoding="UTF-8"?>
<document exportedAt="${escapeXml((/* @__PURE__ */ new Date()).toISOString())}">
  <metadata>
    <characters>${markdown2.length}</characters>
    <words>${markdown2.trim().split(/\s+/).filter(Boolean).length}</words>
    <lines>${markdown2.split("\n").length}</lines>
  </metadata>
  <headings>
${headings.map((heading2) => `    <heading level="${heading2.level}">${escapeXml(heading2.text)}</heading>`).join("\n")}
  </headings>
  <paragraphs>
${paragraphs.map((paragraph2) => `    <paragraph>${escapeXml(paragraph2)}</paragraph>`).join("\n")}
  </paragraphs>
  <sourceMarkdown><![CDATA[${markdown2.replaceAll("]]>", "]]]]><![CDATA[>")}]]></sourceMarkdown>
</document>`;
}
function markdownToRtf(markdown2) {
  const text2 = markdownToPlainText(markdown2);
...
function htmlToMarkdown(html2) {
  ...
  const content2 = Array.from(doc.body.childNodes).map((node2) => nodeToMarkdown(node2)).join("").replace(/\n{3,}/g, "\n\n").trim();
  return content2 || doc.body.textContent?.trim() || "";
}
function nodeToMarkdown(node2, depth = 0) {
  if (node2.nodeType === Node.TEXT_NODE) {
    return node2.textContent?.replace(/\s+/g, " ") ?? "";
  }
  ...
  const children = Array.from(element2.childNodes).map((child) => nodeToMarkdown(child, depth)).join("").replace(/[ \t]+\n/g, "\n");
  if (tag === "br") return "\n";
  if (tag === "hr") return "\n---\n\n";
  if (tag === "strong" || tag === "b") return `**${children.trim()}**`;
  if (tag === "em" || tag === "i") return `*${children.trim()}*`;
  if (tag === "code" && element2.parentElement?.tagName.toLowerCase() !== "pre") {
    return `\`${children.trim()}\``;
  }
  if (tag === "pre") {
    const code2 = element2.textContent?.replace(/\n$/, "") ?? "";
    return `
\`\`\`
${code2}
\`\`\`
...
`;
  ...
  if (tag === "img") {
    const alt = element2.getAttribute("alt") || "image";
...
`;
  ...
  if (tag === "ul") {
    return `${Array.from(element2.children).map((li) => `${"  ".repeat(depth)}- ${nodeToMarkdown(li, depth + 1).trim()}`).join("\n")}

`;
  }
  if (tag === "ol") {
    return `${Array.from(element2.children).map((li, index2) => `${"  ".repeat(depth)}${index2 + 1}. ${nodeToMarkdown(li, depth + 1).trim()}`).join("\n")}
...
`;
  ...
  if (tag === "table") {
    return `${tableElementToMarkdown(element2)}
...
function delimitedTextToMarkdownTable(raw, delimiter) {
  const rows = parseDelimitedText(raw, delimiter);
  if (rows.length === 0) {
    return "";
  }
  const normalized = rows.map((row2) => row2.map((cell2) => cell2.trim()));
  const header2 = normalized[0];
  const separator = header2.map(() => "---");
  const body = normalized.slice(1);
  return [header2, separator, ...body].map((row2) => `| ${row2.map((cell2) => cell2.replace(/\|/g, "\\|")).join(" | ")} |`).join("\n");
}
function parseDelimitedText(raw, delimiter) {
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    if (!inQuotes && (char === "\n" || char === "\r")) {
      ...
      row2.push(cell2);
      if (row2.some((item) => item.length > 0)) {
        rows.push(row2);
      }
      row2 = [];
      cell2 = "";
...
function extractMarkdownTables(markdown2) {
  const lines = markdown2.split("\n");
  const tables = [];
  let buffer = [];
  const flush = () => {
...
function parseMarkdownTable(lines) {
  const rows = lines.map((line) => line.trim()).filter(Boolean).map((line) => {
    const trimmed = line.replace(/^\||\|$/g, "");
    return trimmed.split("|").map((cell2) => cell2.trim());
  });
  return rows.filter((row2, index2) => {
    if (index2 !== 1) {
      return true;
    }
    return !row2.every((cell2) => /^:?-{3,}:?$/.test(cell2));
  });
}
function tableElementToMarkdown(table2) {
  const rows = Array.from(table2.querySelectorAll("tr")).map(
    (row2) => Array.from(row2.querySelectorAll("th, td")).map(
      (cell2) => (cell2.textContent ?? "").trim().replace(/\|/g, "\\|")
    )
  );
  if (rows.length === 0) {
    return "";
  ...
  return [rows[0], separator, ...rows.slice(1)].map((row2) => `| ${row2.join(" | ")} |`).join("\n");
}
function recordsToMarkdownTable(records) {
  ...
  const rows = records.map(
    (record) => headers.map((header2) => String(record[header2] ?? "").replace(/\|/g, "\\|"))
  );
  return [headers, separator, ...rows].map((row2) => `| ${row2.join(" | ")} |`).join("\n");
}
...
function escapeRtf(value) {
  return value.replace(/\\/g, "\\\\").replace(/\{/g, "\\{").replace(/\}/g, "\\}").replace(/[^\x00-\x7F]/g, (char) => `\\u${char.charCodeAt(0)}?`);
}
function Toolbar({
  onInsert,
  onLoadTemplate,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "toolbar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "toolbar-group", children: formatActions.map((action, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "toolbar-group", children: blockActions.map((action, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        className: "toolbar-btn",
        title: action.title,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "toolbar-divider" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "toolbar-group", children: insertActions.map((action, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        className: "toolbar-btn",
        title: action.title,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "template-btn", style: { cursor: "pointer", marginRight: "8px" }, children: [
      t.upload,
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "toolbar-formats", children: t.uploadFormats }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "template-selector", ref: dropdownRef, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      showTemplates && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "template-dropdown", children: templates.map((t2, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "template-item",
          onClick: () => {
            onLoadTemplate(t2.content);
            setShowTemplates(false);
          },
          id: `template-item-${i}`,
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "template-item-icon", children: t2.icon }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "template-item-info", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "template-item-name", children: t2.name }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "template-item-desc", children: t2.description })
            ] })
          ]
        },
        i
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        ...
        children: isExporting ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
          ...
          t.exporting
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: t.export })
      }
    )
  ] });
}
...
function dynamic(loader, options) {
  ...
  if (!ssr) {
    ...
    const LazyComponent2 = reactExports.lazy(async () => {
    ...
    const ClientSSRFalse = (props) => {
      ...
      return React.createElement(reactExports.Suspense, { fallback }, React.createElement(LazyComponent2, props));
    };
    ClientSSRFalse.displayName = "DynamicClientSSRFalse";
    return ClientSSRFalse;
  }
  if (isServer) {
    const LazyServer = reactExports.lazy(async () => {
      const mod = await loader();
      if ("default" in mod)
        return mod;
      return { default: mod };
    ...
    const ServerDynamic = (props) => {
      ...
      const content2 = ErrorBoundary2 ? React.createElement(ErrorBoundary2, { fallback: LoadingComponent }, lazyElement) : lazyElement;
      return React.createElement(reactExports.Suspense, { fallback }, content2);
    };
    ServerDynamic.displayName = "DynamicServer";
    return ServerDynamic;
  }
  const LazyComponent = reactExports.lazy(async () => {
    const mod = await loader();
    if ("default" in mod)
      return mod;
    return { default: mod };
  ...
  const ClientDynamic = (props) => {
    const fallback = LoadingComponent ? React.createElement(LoadingComponent, { isLoading: true, pastDelay: true, error: null }) : null;
    return React.createElement(reactExports.Suspense, { fallback }, React.createElement(LazyComponent, props));
  };
  ClientDynamic.displayName = "DynamicClient";
  return ClientDynamic;
}
function Editor({ value, onChange, theme, onEditorViewReady }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(EditorInner, { value, onChange, theme, onEditorViewReady });
}
...
const ClientEditor = dynamic(
  () => import("./EditorClient-C8lkuedL.js").then((n) => n.w),
  {
    ssr: false,
...
const Preview = reactExports.forwardRef(
  ({ markdown: markdownContent }, ref) => {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preview-content", ref, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      Markdown,
      {
...
function ExportModal({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "modal-title", children: t.title }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "modal-close", onClick: onClose, id: "modal-close-btn", children: "✕" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", htmlFor: "filename-input", children: t.filename }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            id: "filename-input",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", htmlFor: "format-select", children: t.format }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "select",
          {
            id: "format-select",
            ...
            children: EXPORT_FORMATS.map((format) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: format.value, children: t.formats[format.value] }, format.value))
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "form-help", children: t.formatHint })
      ] }),
      settings.format === "pdf" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "form-section-label", children: t.pdfOptions }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", htmlFor: "pagesize-select", children: t.pageSize }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "select",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "a4", children: t.sizeA4 }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "letter", children: t.sizeLetter })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", htmlFor: "margin-input", children: t.margin }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                id: "margin-input",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-group", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", htmlFor: "quality-select", children: t.quality }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "select",
            {
              id: "quality-select",
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "0.8", children: t.qualityStandard }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "0.95", children: t.qualityHigh }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "1", children: t.qualityMax })
              ]
            }
          )
        ] })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-footer", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-secondary", onClick: onClose, id: "modal-cancel-btn", children: t.cancel }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          disabled: isExporting,
          id: "modal-export-btn",
          children: isExporting ? t.generating : t.download
        }
      )
    ] })
  ] }) });
...
function Home(props) {
  const params = reactExports.use(props.params);
  const locale = params.locale;
  const { t } = useTranslation(locale);
  const [markdownContent, setMarkdownContent] = reactExports.useState(t.welcome);
    ...
    (text2) => {
      ...
      if (view) {
        ...
        view.dispatch({
          ...
          selection: { anchor: selection.from + text2.length }
        });
        view.focus();
      } else {
        setMarkdownContent((prev) => prev + text2);
      }
    },
    []
  );
  const handleLoadTemplate = reactExports.useCallback((content2) => {
    setMarkdownContent(content2);
  }, []);
  ...
  const handleLoadFile = reactExports.useCallback(
    async (file) => {
      try {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      onDrop: handleDrop,
      children: [
        isDragging && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "drag-overlay", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "drag-message", children: t.dragdrop.message }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Header, { theme, onToggleTheme: toggleTheme, locale, t: t.header }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Toolbar,
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "editor-area", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "editor-panel", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-header", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.editor.title }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-header-right", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "word-count", children: [
                stats.words,
                " ",
                t.editor.words,
                " · ",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "editor-wrapper", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              Editor,
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preview-panel", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-header", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.preview.title }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preview-wrapper", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Preview, { ref: previewRef, markdown: markdownContent }) })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-bar", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "status-bar-left", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-indicator", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "status-dot" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.statusBar.ready })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-bar-right", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: sourceFormat }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.statusBar.multiExport }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.statusBar.encoding }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t.statusBar.shortcut })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          ExportModal,
        ...
        toast2 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `toast toast-${toast2.type}`, children: [
          toast2.type === "success" ? "✅" : "❌",
          " ",
          toast2.message
        ] })
